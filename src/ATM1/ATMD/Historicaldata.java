...
public class Historicaldata extends JFrame {
	static Historicaldata frame4=new Historicaldata();
	...
	void init() {
		...
		try {
			...
			// 获取查询结果集
			ResultSet rs = stmt.executeQuery("SELECT date,取,存,对方账号 FROM atmbankdatabase.atm");
			//列表数组
			String[] columnNames = {"交易日期", "支出", "收入", "对方账号"};
			//查询数据
			    model=new DefaultTableModel();//表格关键人物 位置不能变 只能在这里
				String sql1="select * from atmbankdatabase.atm ";
